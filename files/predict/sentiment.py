...
class VecReader:
    def __init__(
        self,
        indexfile=os.path.dirname(__file__) + "/s24_surface_vecs.hofst",
        vecfile=os.path.dirname(__file__) + "/binary_vecs.bin",
        dim=128,
    ):
    ...
    def find(self, word):
        ...
        vec = np.fromfile(self.vecs, dtype=np.float16, count=self.dimension)
        self.cache[word] = vec
        return vec
...
class CNN_Text(nn.Module):
    ...
    def embed(self, x):
        ...
        maxlen = max(max(map(len, x)), max(self.args.kernel_sizes))
...
class Args:
    def __init__(self):
        ...
        self.snapshot = (
            ...
            + "/snapshot/2018-08-16_06-35-21/best_steps_2700.pt"
        )
        self.predict = None
        self.test = False
        self.class_num = 3
...
def train(train_iter, dev_iter, model, args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        while batch_num * args.batch_size < len(train_iter):
            batch = train_iter[
                batch_num * args.batch_size : (batch_num + 1) * args.batch_size
            ]
            batch_num += 1
            feature, target = list(map(lambda x: x[0], batch)), torch.LongTensor(
                list(map(lambda x: int(x[1]) - 1, batch))
            )
            ...
            if steps % args.log_interval == 0:
                ...
                sys.stdout.write(
                    "\rBatch[{}] - loss: {:.6f}  acc: {:.4f}%({}/{})".format(
                        steps, loss.data[0], accuracy, corrects, len(batch)
                    )
                )
...
def eval(data, model, args):
    model.eval()
    feature, target = list(map(lambda x: x[0], data)), torch.LongTensor(
        list(map(lambda x: int(x[1]) - 1, data))
    )
    logit = model(feature)
...
def train_predict_batch(texts, model):
    model.eval()
    # text = text_field.tokenize(text)
    #    text = text_field.preprocess(text)
    #    text = [[text_field.vocab.stoi[x] for x in text]]
...
def predict_batch(texts):
    ...
    resultnames = ["negative", "neutral", "positive"]
    return list(map(lambda x: resultnames[x], train_predict_batch(texts, cnn)))
...
def predict(texts):
    try:
        ...
        return (str(ex), exc_type, fname, exc_tb.tb_lineno)
        ...
        str(
            predict(
                [
                    ["Tosi", "hyvä", "juttu", "!"],
                    ["Tosi", "huono", "juttu", "!"],
                    ["Elokuva", "oli", "miellyttävä", "."],
                    ["Elokuva", "oli", "puuduttava", "."],
                    ["Elokuva", "oli", "ihan", "ok."],
                    [
                        "Tosi",
                        "hyvä",
