...
def html_from_conllu(conllu):
    ...
    response = requests.post(kielipankki_conllu_api_url, data=encoded_text, params={})
...
def conllu_from_text(text):
    ...
    response = requests.post(kielipankki_parse_api_url, data=encoded_text, params={})
    response.raise_for_status()
    return response.text
...
def print_content():
    ...
    if inputval != "":
        ...
        """.format(
            parsed_html_file="https://kielipankki.fi/tools/demo/tmp/"
            + session_key
            + ".html"
        )
...
""".format(
    ...
    sys.stdout.buffer.write(
        wrap_html(
            make_head(title="finparse demo", scripts=(populate_js, remember_js)),
            wrap_in_tags(body, "div", attribs='class="container pt-1"', oneline=False),
            onload="rememberInput()",
        ).encode("utf-8")
    )
