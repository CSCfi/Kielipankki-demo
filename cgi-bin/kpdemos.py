...
def mx_auto(t):
    return wrap_in_tags(
        wrap_in_tags(t, "div", attribs='class="col center-block"'),
        "div",
        attribs='class="row mx-auto"',
    )
...
def text_from_file(form_file):
    ...
    if ext in (".png", ".jpg", ".jpeg", ".gif", ".tiff"):
        ...
        retval = textract.process(
            uploaded_file_path, encoding="utf-8", extension=ext[1:]
        )
    retval = re.sub(
        r"[\x00-\x08\x0b\x0c\x0e-\x1f\x7f-\x9f]", "", str(retval, encoding="utf-8")
    ).encode("utf-8")
...
def make_table(rows, header=[], tdattribs=""):
    ...
    def escape(s):
        if s.startswith('<img src="' + hostname) or s.startswith(
            '<img src="' + kp_hostname
        ):
            return s
        return xml_escape(s)
    ...
    for row in rows:
        ...
        for item in row:
            this_row += wrap_in_tags(
                escape(nice_format(item)), "td", attribs=tdattribs.format(CELL=item)
            )
...
def nertag(text, show_analyses=False):
    ...
    response = requests.post(
        submit_url, data=encoded_text, params={"show-analyses": show_analyses}
    )
