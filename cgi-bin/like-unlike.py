...
def make_cluster(clusterword, vecs):
    ...
    for word2 in words:
        if word2 != clusterword:
            clusterwords_with_weights = vecs.like(
                clusterword, word2, nwords, similarityfactor
            )
    ...
    for key in groups:
        ...
        for key2 in groups:
            if key == key2:
                continue
            associations[key][key2] = len(
                list(filter(lambda x: x in groups[key2], groups[key]))
            )
    ...
    nx.draw_networkx_edges(G, pos, edgelist=edges, width=1, alpha=0.1, edge_color="b")
    ...
    nx.draw_networkx_labels(
        G, add_to_verticals(pos), font_size=10, font_family="sans-serif"
    )
    ...
    writefile = tmpdir + "/like_cluster_for_" + asciiclusterword + ".png"
    linkaddress = hostname + "/tmp/like_cluster_for_" + asciiclusterword + ".png"
...
""".format(
    ...
    sys.stdout.buffer.write(
        wrap_html(
            make_head(title="like-unlike demo", scripts=(populate_js,)),
