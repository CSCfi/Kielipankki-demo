            ...
            for section in json_result["sections"]:
                ...
                for word in section["words"]:
                    words.append(
                        [
                            word["word"],
                            "{:.2f}".format(start_time + float(word["start"])),
                            "{:.2f}".format(start_time + float(word["end"])),
                        ]
                    )
                    if done:
...
""".format(
    ...
    if jobid is None or done == True:
        ...
        head = make_head(
            title="asr demo",
            extra_meta='<meta http-equiv="refresh" content="2; url=asr.py{refresh_target}">'.format(
