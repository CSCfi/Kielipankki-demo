...
def print_content():
    ...
    if inputval != "":
        ...
        try:
            ...
            #            return
            texts_and_results = list(
                zip(results, map(lambda x: " ".join(x), sentences))
            )
            result += "<p>Result:</p>\n"
            result += """
<div class="row">
  <div class="col-md-auto">
    ...
    <a class="btn btn-info" href="{html_root}/kielipankki-tools/tmp/{filename}.xlsx" download="finnsentiment_result.xlsx" role="button">Download Excel spreadsheet</a>
            ...
            sys.stdout.buffer.write(
                wrap_html(
                    make_head("FinnSentiment demo error"),
                    wrap_in_tags(
                        errinfo + "<p>Got exception " + str(ex) + "</p>\n",
                        "body",
                        oneline=False,
                    ),
                ).encode("utf-8")
...
""".format(
    ...
    sys.stdout.buffer.write(
        wrap_html(
            make_head("FinnSentiment demo", scripts=(remember_input_js, demo_js)),
            wrap_in_tags(
