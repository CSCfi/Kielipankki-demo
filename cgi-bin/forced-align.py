...
def print_content():
    ...
    if "audio-file" in form and "transcript-file" in form:
        ...
        files = {
            "audio": (form["audio-file"].filename, form["audio-file"].file),
            "transcript": (
                form["transcript-file"].filename,
                form["transcript-file"].file,
            ),
        ...
        submit_url = "http://kielipankki.2.rahtiapp.fi/audio/align/fi/submit_file"
        try:
    ...
    if jobid is not None:
        ...
        if "status" in j and j["status"] == "pending":
            ...
            result += (
                wrap_in_tags("Preview", "h3")
                + make_table(preview_words, header=columns)
                + "\n"
            )
...
""".format(
    ...
    if jobid is None or done == True:
        ...
        head = make_head(
            title="align demo",
            extra_meta='<meta http-equiv="refresh" content="2; url=forced-align.py{refresh_target}">'.format(
                refresh_target=target_params
            ),
        )
